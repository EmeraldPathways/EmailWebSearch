...
2. Create/edit service in `src/services/` — all Apify calls go through `runActor()`
...
## Apify data
- Always use `outputFormat: 'full'`
- Always use RESIDENTIAL proxy
- Cache via `runActor()` with appropriate TTL key

## After implementation
- Test locally at `http://localhost:3001`
- Run `npm run build`
- Deploy with `/project:deploy` or the deploy skill
